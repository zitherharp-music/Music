...
public class MainFragment extends Fragment {
    ...
    private Song song1, song2, song3, playingSong;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        playlistString.setOnFocusChangeListener((v, hasFocus) -> {

        });
        playingSong = (Song) MusicHelper.getRandomItem(MusicProvider.getSongs());
        playerView.getYouTubePlayerWhenReady(youTubePlayer -> {
            youTubePlayer.loadVideo(playingSong.getId(), 0);
            youTubePlayer.addListener(new YouTubePlayerListener() {
                ...
                public void onStateChange(@NotNull YouTubePlayer youTubePlayer, @NotNull PlayerConstants.PlayerState playerState) {
                    if (playerState.equals(PlayerConstants.PlayerState.ENDED)) {
                        playingSong = viewModel.getPlaylist().getValue().get(0);
                        youTubePlayer.loadVideo(playingSong.getId(), 0);
                        playingSongDetail.setText(MusicHelper.getSongDetail(playingSong, language));
                        viewModel.play();
                    }
                }
        ...
        viewModel.getPlaylist().observe(getViewLifecycleOwner(), songs -> {
            ...
            song2 = songs.get(1);
            song3 = songs.get(2);
            songId1.setText(song1.getId());
            songId2.setText(song2.getId());
            songId3.setText(song3.getId());
            playlistString.setText(MusicHelper.toString(songs.subList(0, 14), language));
            MusicUtils.loadImage(getActivity(), songPhoto1, song1.getPhotoUrl(PhotoQuality.MQDEFAULT));
            MusicUtils.loadImage(getActivity(), songPhoto2, song2.getPhotoUrl(PhotoQuality.MQDEFAULT));
            MusicUtils.loadImage(getActivity(), songPhoto3, song3.getPhotoUrl(PhotoQuality.MQDEFAULT));
